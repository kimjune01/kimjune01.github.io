...
An agent system needs the same ordering. Short-term memory is the working set that salience maintains during a session. Long-term memory is the decisions, patterns, and preferences that persist across sessions because they won enough rounds of competition to earn consolidation. I thought the consolidation problem was largely solved. [Mem0](https://github.com/mem0ai/mem0) handles automatic extraction, merging, and decay. [Zep](https://www.getzep.com/) builds temporal knowledge graphs that track how facts change over time. That looked like consolidation. It's [compaction](/consolidation) — cache housekeeping that reorganizes what's stored without changing how the system processes the next cycle. The whole industry makes this mistake: every tool that calls itself "memory management" is doing compaction, not consolidation. Both consolidation and competition remain unsolved.
...
So maybe I'm wrong. Maybe the competitive salience layer costs more than it's worth in a practical system, and scoring and ranking is good enough. Caret Recorder handles perception. Cord handles coordination. Memory is pluggable: pick Mem0 or Zep and compaction is handled. The open problems are salience and [consolidation](/consolidation), and the open question is whether salience needs to be competitive at all.
