...
Exchanges that enforce bonds and tenure have better inventory. When each position is backed by either a commercial identity or locked capital, squatters are priced out and the remaining advertisers are differentiated.

DSPs care because differentiated inventory converts better. A user asking about climbing-specific physical therapy gets a climbing PT, not a generic franchise that bought every keyword. Higher relevance means higher click-through rates, which means the DSP's advertiser gets more value per dollar, which means the DSP wins more business from that advertiser. The DSP doesn't route to the exchange with the most inventory. It routes to the exchange where its ads perform best. An exchange full of squatted positions is an exchange where every ad looks the same and none of them convert.

The exchange without these mechanisms gets exactly that: thirty flags from one franchise, all serving generic responses regardless of the query. DSPs route demand elsewhere.
