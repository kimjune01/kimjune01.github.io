...
This post is a map. It surveys who is working on what and where the threads connect.

## The Auction Mechanism

Recent papers tackle the core question: how do you run an auction when the inventory is an LLM conversation rather than a keyword slot?
...
Alibaba's Taobao & Tmall Group took a different approach. **Zhao et al. (2025)** introduced [LLM-Auction](https://arxiv.org/abs/2512.10551), the first generative auction where the LLM itself is the allocation rule. Rather than selecting from pre-existing ad slots, the system integrates auction outcomes into LLM generation using an Iterative Reward-Preference Optimization (IRPO) algorithm. The auction object is the distribution over LLM outputs; the model's response *is* the ad allocation. This is also the approach that is structurally impossible to audit. The allocation rule is baked into the model weights. Advertisers cannot verify why their ad was or was not shown. Users cannot verify that response quality was preserved. The platform can optimize for whatever it wants and call it "relevance," and no one outside the company can prove otherwise.

Both papers build on foundations laid by **Liu et al. (2021)** in [Neural Auction](https://arxiv.org/abs/2106.03593), published at KDD 2021. Their Deep Neural Auctions (DNAs) use differentiable relaxations of discrete sorting to enable end-to-end auction learning. The system was deployed in Taobao's production advertising, where learned auction mechanisms outperformed hand-designed ones at scale. Neural Auction didn't target LLM conversations (it targeted e-commerce display), but it established that embedding-space representations and neural network-based allocation can replace the hand-crafted rules of traditional auctions.
...
- **Academics** proved that auctions can clear in embedding space (Hajiaghayi et al., Zhao et al., Liu et al.)
- **Industry labs** already run production-grade embedding infrastructure for intent matching (Criteo, Twitter/X, Google)
- **Standards bodies** drafted TEE specifications and formed working groups for AI-era monetization (IAB Tech Lab, Google Privacy Sandbox)
- **Platform companies** control the inventory and the users (OpenAI, Google, Perplexity)
- CloudX built TEE attestation for auction transparency specifically
- Open-weight models from Nomic, BGE, and GTE make the embedding step independently verifiable

The complete architecture requires all these pieces at once. The academic papers assume a trusted auctioneer and stop there. Industry infrastructure collapses to categorical targeting at the protocol boundary. TEE work so far addresses privacy, leaving auction transparency to CloudX alone. Platform companies are building walled gardens.
