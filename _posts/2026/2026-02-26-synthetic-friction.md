...
Each cluster has one generalist and four specialists. Specialists get narrow targeting (σ=0.30) and high conversion value (12.0). Generalists get wide reach (σ=0.55) and moderate value (6.0). VCG payments are capped at the winner's value, so no advertiser pays more than an impression is worth.
...
This is the convergence trap. The embedding space has the *capacity* for differentiation. It can tell the climbing PT from the sports PT. But the competitive dynamics on top of that space erase the differentiation over time.
...
Relocation fees are the embedding-space instance of a pattern that appears wherever frictionless repositioning would destroy differentiation.
...
Density-adaptive fees (Cell E) provided zero measurable benefit over uniform fees (Cell D) across all 5 clusters (all p > 0.82). The fee works because it prevents drift everywhere. A single λ works equally well whether the cluster is yoga studios at cosine 0.97 or unrelated tradespeople at cosine 0.53.
