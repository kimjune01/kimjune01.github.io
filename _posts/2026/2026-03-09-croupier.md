...
The relay protocol is open source. A single advertiser and a single publisher can self-host, pass coupons directly, and count conversions without an exchange. The protocol works for one pair.

The exchange earns its keep at many-to-many. Running the [auction](/power-diagrams-ad-auctions) across hundreds of advertisers and publishers, aggregating [distance histograms](/set-it-and-forget-it), maintaining the [public ledger](#keeping-score), hosting both leaderboards. The batch signature is its claim to a cut: every stamped batch is a metered facilitation event. The exchange can bill per batch, per redemption, or a flat monthly fee.

The advertiser's keys and coupons are portable. If the exchange overcharges, the advertiser takes their keys to a competitor or self-hosts for their direct partnerships. [Switching cost](/transparency-is-irreversible) is near zero for the protocol. The exchange earns retention by running better auctions, not by locking in data.
