...
| Action | After strip | After condensation | Classification |
|:---|:---|:---|:---|
| Read an academic article | Article text (first visit) | — | **Context** |
| Open a new docs tab | Docs text (first visit) | — | **Context** |
| Type code in editor | Code diff | Lines written | **Moment** |
| Run a terminal command | Commands + output | Commands + key output | **Moment** |
| Scroll YouTube shorts | Nearly empty | — | Dropped |
| Browse memes | Image captions (first visit) | — | Dropped |

Context is a first visit with natural language: what the user was reading. Moments are condensed return-visit diffs: what the user produced. Events that the stripper empties never reach the LLM.
...
- Salience — moments compete; winners surface, losers decay (hypothesis: [Learn to Forget](/learn-to-forget))
