...
Perplexity [tried ads and killed them](/you-were-right-to-kill-ads). Total revenue: $20,000. Their CEO's reasoning: *a user needs to believe this is the best possible answer.* But Perplexity still has the survival problem. Killing ads is a deferral, not a model. Anthropic's "no ads" positioning doesn't pay for inference either.
...
When the user taps it, the chatbot sends the current conversation context — as an embedding, inside a [sealed TEE enclave](/you-were-right-to-kill-ads) — to the exchange. The exchange returns relevant ads in a dedicated panel, separate from the conversation.
...
A platform could flag every impression as opt-in and collect the premium. The [TEE](/you-were-right-to-kill-ads) attests the auction code. It doesn't attest the UI. The button tap happens in the platform's application layer, outside the enclave. A self-reported flag is worthless.
...
The tradeoffs are real. But compared to forced ads — or to running out of money — it's the option that respects the user most while still paying for the compute they're using.
