...
The prose spec carries architecture, intent, and the *why* behind design choices. Code tells you what the system does, never what it should do. Without the spec, the LLM guesses intent. It guesses wrong.

The prototype encodes decisions the prose left abstract: merge thresholds, similarity functions, lifecycle details. Depending on what you're building, this could be a standalone implementation or an [experiment](https://github.com/kimjune01/union-find-compaction/blob/master/EXPERIMENT.md) that validates the design. Ours was ~300 lines of Python.

The authority rule collapses ambiguity. One sentence that says what wins when the inputs conflict. Without it, the LLM asks you to resolve every decision upfront. Or worse, guesses.

And two models instead of one? Different training, different biases, different blind spots. They fail on different sides of the same boundary. You can't get that from running the same model twice.
