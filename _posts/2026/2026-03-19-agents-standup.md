...
4. **[Blind-blind-merge](/blind-blind-merge) as exploration.** Parallel rollouts, no coordination, merge at the root. The diversity is structural, not stochastic.
...
## Three artifacts, one backward pass
...
**Backward pass (serial, by stage):**

- *Consolidate → Perceive′:* Update what to *notice*. If three agents independently flagged a failure mode, add a perceive hook that catches it at intake.

- *Consolidate → Filter′:* Update what to *reject*. Convergent mistakes are high-confidence filter rules, but only when agents arrived at the lesson independently. Shared failure from shared tooling is herd error, not signal. The test: did agents with different configs converge?

- *Consolidate → Attend′:* Update what to *prioritize*. Convergent lessons are signal, idiosyncratic ones decay.
...
- *Staleness*: `last_fired < N cycles` → evict.
- *Redundancy*: two rules within ε → merge into one. Compression, not deletion.
- *Causation*: does this rule *cause* better outcomes or *correlate* with them? [The blank cell](/the-missing-parts#filter-the-causal-row), bounded causal filtering, is exactly what rule eviction needs.
...
Back to the office. A project starts. The team isn't random: the senior engineer who knows the legacy system, the new hire who'll ask why it works that way, the contractor who's seen how three other companies solved it. The composition is deliberate. The stochasticity comes from what each person notices, not from who they are.
...
| Agent | Tier | Deep stack | Convergence |
|---|---|---|---|
| frontend-ux | Specialist | React, component testing, a11y linting | Tight: lighthouse scores |
| backend-infra | Specialist | Express, Postgres, auth | Loose: integration tests pass |
| security-audit | Rotation | Input validation, dependency audit | Aggressive: reject early |

| Agent | Work log entry |
|---|---|
| frontend-ux | Pet photo upload accepts BMP and TIFF but browsers only render JPEG and GIF. Added server-side conversion. 40 min to find the codec issue. |
| backend-infra | `PUT /pets/:id` accepts any payload shape. Someone uploaded a sock puppet as a pet. Added [Zod](https://zod.dev/) schema validation after a malformed update corrupted a test record. |
| security-audit | Pet photo endpoint accepts files without size limits. Added 5MB cap. Also: backend-infra's Zod schemas don't sanitize HTML in pet descriptions. Flagged. |
...
The BMP/TIFF finding from frontend-ux doesn't converge with anything. Decay counter starts. If it recurs in a future cycle, it earns promotion. If not, it evicts.
...
Work logs stop being documentation and become training data for the live system. The learning is externalized, inspectable, forkable: [open prose](/open-prose) all the way down. Copyleft for cognition: every lesson one agent learns propagates to every agent that shares the baseline.
