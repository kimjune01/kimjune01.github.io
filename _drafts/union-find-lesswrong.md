...
Every LLM chatbot has the same context problem: conversations grow, the window doesn't. The standard fix is flat summarization — run a cheap model over everything outside a recent window, replace the originals with the summary. This works, but it's a lossy, all-or-nothing compression. The summary can't trace a claim back to a specific message. You can't expand a summary to recover what was said. And the whole conversation stalls while the summarizer runs.

I built and tested an alternative that uses [union-find](https://en.wikipedia.org/wiki/Disjoint-set_data_structure) (disjoint-set forests) as the backbone for context compaction. The core idea: instead of compressing 200 messages into one summary, group similar messages into equivalence classes and summarize per-cluster. The union-find structure gives you provenance, recoverability, and incremental updates for free.
...
Five properties distinguish this from flat summarization:
...
4. **Cheaper.** Small clusters mean smaller prompts. Haiku-class models produce good per-cluster summaries.
...
**What flat drops:** Flat summarization preserves headline facts (the database version, the auth method) and drops footnote facts (the scrape interval, the cron schedule, the webhook path). The model decides what matters in a single pass, and footnotes lose. Union-find compresses per-cluster, so the cron schedule doesn't compete against the database version — they live in different clusters.

**Honest accounting:** One significant trial out of seven. The consistent directional advantage is suggestive, but a larger study (200+ paired questions) would be needed to confirm it. I think the architectural argument — provenance, recoverability, incremental updates — matters more than the recall numbers, but reasonable people could disagree.

## What this enables

The forest serializes as parent pointers. Three applications follow:

**Cross-session memory.** Prior conversations arrive as pre-merged clusters, not a blank window. No pinned notes, no "as we discussed last time."

**Multiplayer context.** Two people feed the same forest. Both can query it. Shared memory without shared prompts.

**Branching.** Fork the forest for a what-if conversation. The fork inherits all context.
