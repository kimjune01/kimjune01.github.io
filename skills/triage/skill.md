...
1. Fetch all open PRs and issues:
   - `gh pr list --repo <repo> --state open --json number,title,author,labels,reviewDecision,statusCheckRollup,comments,updatedAt,isDraft`
...
| You got banned or warned on this repo | Cooldown active (check `/consolidate`) |
