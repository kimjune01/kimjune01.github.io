...
Dir.glob('_posts/**/*.md') do |file|
  ...
  if !front_matter.key?('image') || front_matter['image'].nil? || front_matter['image'].empty?
    # Try to match Markdown image syntax
    match = body.match(/!\[.*?\]\((.*?)\)/)
    # If no Markdown image, try to match HTML img tag
    match = body.match(/<img.*?src=["'](.*?)["']/) if !match
